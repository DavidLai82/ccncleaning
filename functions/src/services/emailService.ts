...
The CC&N's Cleaning Team`
  ...
  async sendAppointmentConfirmation(userEmail: string, userName: string, appointmentDetails: AppointmentDetails): Promise<void> {
...
info@ccnscleaning.com | (813) 219-1920`
  ...
  private async sendEmail(options: EmailOptions): Promise<void> {
    try {
      const emailConfig = process.env.EMAIL_USER || functions.config().email?.user;
      ...
      await this.transporter.sendMail({
        from: `"CCN Cleaning Services" <${emailConfig}>`,
        ...options
      });
