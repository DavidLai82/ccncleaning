"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (k !== "default" && Object.prototype.hasOwnProperty.call(mod, k)) __createBinding(result, mod, k);
    __setModuleDefault(result, mod);
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.databaseService = exports.DatabaseService = void 0;
const admin = __importStar(require("firebase-admin"));
const database_1 = require("../config/database");
const functions = __importStar(require("firebase-functions"));
class DatabaseService {
    constructor() { }
    static getInstance() {
        if (!DatabaseService.instance) {
            DatabaseService.instance = new DatabaseService();
        }
        return DatabaseService.instance;
    }
    // USER OPERATIONS
    async createUser(userData) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const userRef = firestore.collection('users').doc();
                const timestamp = admin.firestore.FieldValue.serverTimestamp();
                const userDoc = Object.assign(Object.assign({}, userData), { createdAt: timestamp, updatedAt: timestamp });
                await userRef.set(userDoc);
                // Also write to Supabase for backup
                await this.writeToSupabaseUser(userRef.id, Object.assign(Object.assign({}, userData), { id: userRef.id }));
                return Object.assign(Object.assign({ id: userRef.id }, userData), { created_at: timestamp });
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .insert([Object.assign(Object.assign({}, userData), { created_at: new Date().toISOString(), updated_at: new Date().toISOString() })])
                    .select()
                    .single();
                if (error)
                    throw error;
                // Convert Supabase format to our format
                return this.convertSupabaseUser(data);
            }
        }
        catch (error) {
            functions.logger.error(`Error creating user with ${dbType}:`, error);
            // Try fallback database
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            functions.logger.info(`Attempting fallback to ${fallbackType}`);
            return await this.createUserFallback(userData, fallbackType);
        }
    }
    async getUserById(userId) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const userDoc = await firestore.collection('users').doc(userId).get();
                if (!userDoc.exists)
                    return null;
                return Object.assign({ id: userDoc.id }, userDoc.data());
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .select('*')
                    .eq('id', userId)
                    .single();
                if (error || !data)
                    return null;
                return this.convertSupabaseUser(data);
            }
        }
        catch (error) {
            functions.logger.error(`Error getting user with ${dbType}:`, error);
            // Try fallback database
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.getUserByIdFallback(userId, fallbackType);
        }
    }
    async getUserByEmail(email) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const querySnapshot = await firestore
                    .collection('users')
                    .where('email', '==', email)
                    .limit(1)
                    .get();
                if (querySnapshot.empty)
                    return null;
                const userDoc = querySnapshot.docs[0];
                return Object.assign({ id: userDoc.id }, userDoc.data());
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .select('*')
                    .eq('email', email)
                    .single();
                if (error || !data)
                    return null;
                return this.convertSupabaseUser(data);
            }
        }
        catch (error) {
            functions.logger.error(`Error getting user by email with ${dbType}:`, error);
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.getUserByEmailFallback(email, fallbackType);
        }
    }
    async updateUser(userId, updateData) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const userRef = firestore.collection('users').doc(userId);
                await userRef.update(Object.assign(Object.assign({}, updateData), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
                // Also update in Supabase
                await this.updateSupabaseUser(userId, updateData);
                const updatedDoc = await userRef.get();
                return updatedDoc.exists ? Object.assign({ id: updatedDoc.id }, updatedDoc.data()) : null;
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .update(Object.assign(Object.assign({}, updateData), { updated_at: new Date().toISOString() }))
                    .eq('id', userId)
                    .select()
                    .single();
                if (error)
                    throw error;
                return this.convertSupabaseUser(data);
            }
        }
        catch (error) {
            functions.logger.error(`Error updating user with ${dbType}:`, error);
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.updateUserFallback(userId, updateData, fallbackType);
        }
    }
    // APPOINTMENT OPERATIONS
    async createAppointment(appointmentData) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const appointmentRef = firestore.collection('appointments').doc();
                const timestamp = admin.firestore.FieldValue.serverTimestamp();
                const appointmentDoc = Object.assign(Object.assign({}, appointmentData), { createdAt: timestamp, updatedAt: timestamp });
                await appointmentRef.set(appointmentDoc);
                // Also write to Supabase
                await this.writeToSupabaseAppointment(appointmentRef.id, Object.assign(Object.assign({}, appointmentData), { id: appointmentRef.id }));
                return Object.assign(Object.assign({ id: appointmentRef.id }, appointmentData), { createdAt: timestamp });
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('appointments')
                    .insert([Object.assign(Object.assign({}, appointmentData), { appointment_date: appointmentData.appointmentDate, service_type: appointmentData.serviceType, user_id: appointmentData.userId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() })])
                    .select()
                    .single();
                if (error)
                    throw error;
                return this.convertSupabaseAppointment(data);
            }
        }
        catch (error) {
            functions.logger.error(`Error creating appointment with ${dbType}:`, error);
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.createAppointmentFallback(appointmentData, fallbackType);
        }
    }
    async getUserAppointments(userId) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const querySnapshot = await firestore
                    .collection('appointments')
                    .where('userId', '==', userId)
                    .orderBy('createdAt', 'desc')
                    .get();
                return querySnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('appointments')
                    .select('*')
                    .eq('user_id', userId)
                    .order('created_at', { ascending: false });
                if (error)
                    throw error;
                return data.map(this.convertSupabaseAppointment);
            }
        }
        catch (error) {
            functions.logger.error(`Error getting user appointments with ${dbType}:`, error);
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.getUserAppointmentsFallback(userId, fallbackType);
        }
    }
    // PAYMENT OPERATIONS
    async createPayment(paymentData) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const paymentRef = firestore.collection('payments').doc();
                const timestamp = admin.firestore.FieldValue.serverTimestamp();
                const paymentDoc = Object.assign(Object.assign({}, paymentData), { createdAt: timestamp, updatedAt: timestamp });
                await paymentRef.set(paymentDoc);
                // Also write to Supabase for backup
                await this.writeToSupabasePayment(paymentRef.id, Object.assign(Object.assign({}, paymentData), { id: paymentRef.id }));
                return Object.assign(Object.assign({ id: paymentRef.id }, paymentData), { createdAt: timestamp });
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('payments')
                    .insert([Object.assign(Object.assign({}, paymentData), { user_id: paymentData.userId, appointment_id: paymentData.appointmentId, stripe_payment_intent_id: paymentData.stripePaymentIntentId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() })])
                    .select()
                    .single();
                if (error)
                    throw error;
                return this.convertSupabasePayment(data);
            }
        }
        catch (error) {
            functions.logger.error(`Error creating payment with ${dbType}:`, error);
            // Try fallback database
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            functions.logger.info(`Attempting fallback to ${fallbackType}`);
            return await this.createPaymentFallback(paymentData, fallbackType);
        }
    }
    async getUserPayments(userId) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const querySnapshot = await firestore
                    .collection('payments')
                    .where('userId', '==', userId)
                    .orderBy('createdAt', 'desc')
                    .get();
                return querySnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('payments')
                    .select('*')
                    .eq('user_id', userId)
                    .order('created_at', { ascending: false });
                if (error)
                    throw error;
                return data.map(this.convertSupabasePayment);
            }
        }
        catch (error) {
            functions.logger.error(`Error getting user payments with ${dbType}:`, error);
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.getUserPaymentsFallback(userId, fallbackType);
        }
    }
    async updatePaymentStatus(stripePaymentIntentId, status) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const querySnapshot = await firestore
                    .collection('payments')
                    .where('stripePaymentIntentId', '==', stripePaymentIntentId)
                    .limit(1)
                    .get();
                if (!querySnapshot.empty) {
                    const paymentRef = querySnapshot.docs[0].ref;
                    await paymentRef.update({
                        status,
                        updatedAt: admin.firestore.FieldValue.serverTimestamp()
                    });
                }
                // Also update in Supabase
                await this.updateSupabasePaymentStatus(stripePaymentIntentId, status);
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                await supabase
                    .from('payments')
                    .update({
                    status,
                    updated_at: new Date().toISOString()
                })
                    .eq('stripe_payment_intent_id', stripePaymentIntentId);
            }
        }
        catch (error) {
            functions.logger.error(`Error updating payment status with ${dbType}:`, error);
        }
    }
    async updateAppointmentStatus(appointmentId, status) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                await firestore
                    .collection('appointments')
                    .doc(appointmentId)
                    .update({
                    status,
                    updatedAt: admin.firestore.FieldValue.serverTimestamp()
                });
                // Also update in Supabase
                await this.updateSupabaseAppointmentStatus(appointmentId, status);
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                await supabase
                    .from('appointments')
                    .update({
                    status,
                    updated_at: new Date().toISOString()
                })
                    .eq('id', appointmentId);
            }
        }
        catch (error) {
            functions.logger.error(`Error updating appointment status with ${dbType}:`, error);
        }
    }
    // ENHANCED USER OPERATIONS
    async getAllUsers(options) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                let query = firestore.collection('users');
                // Apply role filter
                if (options.role) {
                    query = query.where('role', '==', options.role);
                }
                // Get total count first
                const countSnapshot = await query.get();
                const total = countSnapshot.size;
                // Apply pagination
                query = query.orderBy('createdAt', 'desc')
                    .limit(options.limit)
                    .offset(options.offset);
                const querySnapshot = await query.get();
                let users = querySnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
                // Apply search filter (client-side for Firebase)
                if (options.search) {
                    const searchTerm = options.search.toLowerCase();
                    users = users.filter(user => user.first_name.toLowerCase().includes(searchTerm) ||
                        user.last_name.toLowerCase().includes(searchTerm) ||
                        user.email.toLowerCase().includes(searchTerm));
                }
                return { data: users, total };
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                let query = supabase.from('users').select('*', { count: 'exact' });
                // Apply role filter
                if (options.role) {
                    query = query.eq('role', options.role);
                }
                // Apply search filter
                if (options.search) {
                    query = query.or(`first_name.ilike.%${options.search}%,last_name.ilike.%${options.search}%,email.ilike.%${options.search}%`);
                }
                // Apply pagination
                query = query
                    .order('created_at', { ascending: false })
                    .range(options.offset, options.offset + options.limit - 1);
                const { data, error, count } = await query;
                if (error)
                    throw error;
                return {
                    data: data.map(this.convertSupabaseUser),
                    total: count || 0
                };
            }
        }
        catch (error) {
            functions.logger.error(`Error getting all users with ${dbType}:`, error);
            // Try fallback database
            const fallbackType = dbType === 'firebase' ? 'supabase' : 'firebase';
            return await this.getAllUsersFallback(options, fallbackType);
        }
    }
    async deleteUser(userId) {
        const dbType = await (0, database_1.getActiveDatabaseType)();
        try {
            if (dbType === 'firebase') {
                const firestore = (0, database_1.getFirestore)();
                const batch = firestore.batch();
                // Delete user document
                const userRef = firestore.collection('users').doc(userId);
                batch.delete(userRef);
                // Delete user's appointments
                const appointmentsSnapshot = await firestore
                    .collection('appointments')
                    .where('userId', '==', userId)
                    .get();
                appointmentsSnapshot.docs.forEach((doc) => {
                    batch.delete(doc.ref);
                });
                // Delete user's payments
                const paymentsSnapshot = await firestore
                    .collection('payments')
                    .where('userId', '==', userId)
                    .get();
                paymentsSnapshot.docs.forEach((doc) => {
                    batch.delete(doc.ref);
                });
                await batch.commit();
                // Also delete from Supabase
                await this.deleteSupabaseUser(userId);
            }
            else {
                const supabase = (0, database_1.getSupabase)();
                // Delete user (cascades to appointments and payments due to foreign keys)
                const { error } = await supabase
                    .from('users')
                    .delete()
                    .eq('id', userId);
                if (error)
                    throw error;
            }
            return true;
        }
        catch (error) {
            functions.logger.error(`Error deleting user with ${dbType}:`, error);
            return false;
        }
    }
    async createUserFallback(userData, dbType) {
        if (dbType === 'supabase') {
            const supabase = (0, database_1.getSupabase)();
            const { data, error } = await supabase
                .from('users')
                .insert([Object.assign(Object.assign({}, userData), { created_at: new Date().toISOString(), updated_at: new Date().toISOString() })])
                .select()
                .single();
            if (error)
                throw error;
            return this.convertSupabaseUser(data);
        }
        else {
            const firestore = (0, database_1.getFirestore)();
            const userRef = firestore.collection('users').doc();
            const timestamp = admin.firestore.FieldValue.serverTimestamp();
            const userDoc = Object.assign(Object.assign({}, userData), { createdAt: timestamp, updatedAt: timestamp });
            await userRef.set(userDoc);
            return Object.assign(Object.assign({ id: userRef.id }, userData), { createdAt: timestamp });
        }
    }
    async getUserByIdFallback(userId, dbType) {
        try {
            if (dbType === 'supabase') {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .select('*')
                    .eq('id', userId)
                    .single();
                if (error || !data)
                    return null;
                return this.convertSupabaseUser(data);
            }
            else {
                const firestore = (0, database_1.getFirestore)();
                const userDoc = await firestore.collection('users').doc(userId).get();
                if (!userDoc.exists)
                    return null;
                return Object.assign({ id: userDoc.id }, userDoc.data());
            }
        }
        catch (error) {
            functions.logger.error(`Fallback ${dbType} also failed:`, error);
            return null;
        }
    }
    async getUserByEmailFallback(email, dbType) {
        try {
            if (dbType === 'supabase') {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .select('*')
                    .eq('email', email)
                    .single();
                if (error || !data)
                    return null;
                return this.convertSupabaseUser(data);
            }
            else {
                const firestore = (0, database_1.getFirestore)();
                const querySnapshot = await firestore
                    .collection('users')
                    .where('email', '==', email)
                    .limit(1)
                    .get();
                if (querySnapshot.empty)
                    return null;
                const userDoc = querySnapshot.docs[0];
                return Object.assign({ id: userDoc.id }, userDoc.data());
            }
        }
        catch (error) {
            functions.logger.error(`Fallback ${dbType} also failed:`, error);
            return null;
        }
    }
    async updateUserFallback(userId, updateData, dbType) {
        try {
            if (dbType === 'supabase') {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('users')
                    .update(Object.assign(Object.assign({}, updateData), { updated_at: new Date().toISOString() }))
                    .eq('id', userId)
                    .select()
                    .single();
                if (error)
                    throw error;
                return this.convertSupabaseUser(data);
            }
            else {
                const firestore = (0, database_1.getFirestore)();
                const userRef = firestore.collection('users').doc(userId);
                await userRef.update(Object.assign(Object.assign({}, updateData), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
                const updatedDoc = await userRef.get();
                return updatedDoc.exists ? Object.assign({ id: updatedDoc.id }, updatedDoc.data()) : null;
            }
        }
        catch (error) {
            functions.logger.error(`Fallback ${dbType} also failed:`, error);
            return null;
        }
    }
    async createAppointmentFallback(appointmentData, dbType) {
        if (dbType === 'supabase') {
            const supabase = (0, database_1.getSupabase)();
            const { data, error } = await supabase
                .from('appointments')
                .insert([Object.assign(Object.assign({}, appointmentData), { appointment_date: appointmentData.appointmentDate, service_type: appointmentData.serviceType, user_id: appointmentData.userId, created_at: new Date().toISOString(), updated_at: new Date().toISOString() })])
                .select()
                .single();
            if (error)
                throw error;
            return this.convertSupabaseAppointment(data);
        }
        else {
            const firestore = (0, database_1.getFirestore)();
            const appointmentRef = firestore.collection('appointments').doc();
            const timestamp = admin.firestore.FieldValue.serverTimestamp();
            const appointmentDoc = Object.assign(Object.assign({}, appointmentData), { createdAt: timestamp, updatedAt: timestamp });
            await appointmentRef.set(appointmentDoc);
            return Object.assign(Object.assign({ id: appointmentRef.id }, appointmentData), { createdAt: timestamp });
        }
    }
    async getUserAppointmentsFallback(userId, dbType) {
        try {
            if (dbType === 'supabase') {
                const supabase = (0, database_1.getSupabase)();
                const { data, error } = await supabase
                    .from('appointments')
                    .select('*')
                    .eq('user_id', userId)
                    .order('created_at', { ascending: false });
                if (error)
                    throw error;
                return data.map(this.convertSupabaseAppointment);
            }
            else {
                const firestore = (0, database_1.getFirestore)();
                const querySnapshot = await firestore
                    .collection('appointments')
                    .where('userId', '==', userId)
                    .orderBy('createdAt', 'desc')
                    .get();
                return querySnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
            }
        }
        catch (error) {
            functions.logger.error(`Fallback ${dbType} also failed:`, error);
            return [];
        }
    }
    // BACKUP WRITE METHODS
    async writeToSupabasePayment(id, paymentData) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('payments')
                .upsert({
                id: id,
                user_id: paymentData.userId,
                appointment_id: paymentData.appointmentId,
                amount: paymentData.amount,
                currency: paymentData.currency,
                status: paymentData.status,
                stripe_payment_intent_id: paymentData.stripePaymentIntentId,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
            });
        }
        catch (error) {
            functions.logger.warn('Failed to backup payment to Supabase:', error);
        }
    }
    async updateSupabasePaymentStatus(stripePaymentIntentId, status) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('payments')
                .update({
                status,
                updated_at: new Date().toISOString()
            })
                .eq('stripe_payment_intent_id', stripePaymentIntentId);
        }
        catch (error) {
            functions.logger.warn('Failed to update payment status in Supabase:', error);
        }
    }
    async updateSupabaseAppointmentStatus(appointmentId, status) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('appointments')
                .update({
                status,
                updated_at: new Date().toISOString()
            })
                .eq('id', appointmentId);
        }
        catch (error) {
            functions.logger.warn('Failed to update appointment status in Supabase:', error);
        }
    }
    async deleteSupabaseUser(userId) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('users')
                .delete()
                .eq('id', userId);
        }
        catch (error) {
            functions.logger.warn('Failed to delete user from Supabase:', error);
        }
    }
    async writeToSupabaseUser(id, userData) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('users')
                .upsert({
                id: id,
                email: userData.email,
                first_name: userData.first_name,
                last_name: userData.last_name,
                phone: userData.phone,
                avatar: userData.avatar,
                role: userData.role,
                is_verified: userData.is_verified,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
            });
        }
        catch (error) {
            functions.logger.warn('Failed to backup user to Supabase:', error);
        }
    }
    async updateSupabaseUser(id, updateData) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('users')
                .update(Object.assign(Object.assign({}, updateData), { updated_at: new Date().toISOString() }))
                .eq('id', id);
        }
        catch (error) {
            functions.logger.warn('Failed to update user in Supabase:', error);
        }
    }
    async writeToSupabaseAppointment(id, appointmentData) {
        try {
            const supabase = (0, database_1.getSupabase)();
            await supabase
                .from('appointments')
                .upsert({
                id: id,
                user_id: appointmentData.userId,
                service_type: appointmentData.serviceType,
                appointment_date: appointmentData.appointmentDate,
                status: appointmentData.status,
                address: appointmentData.address,
                notes: appointmentData.notes,
                price: appointmentData.price,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
            });
        }
        catch (error) {
            functions.logger.warn('Failed to backup appointment to Supabase:', error);
        }
    }
    // CONVERSION HELPERS
    convertSupabaseUser(data) {
        return {
            id: data.id,
            email: data.email,
            first_name: data.first_name,
            last_name: data.last_name,
            phone: data.phone,
            avatar: data.avatar,
            role: data.role,
            is_verified: data.is_verified,
            created_at: data.created_at,
            updated_at: data.updated_at
        };
    }
    convertSupabaseAppointment(data) {
        return {
            id: data.id,
            userId: data.user_id,
            serviceType: data.service_type,
            appointmentDate: data.appointment_date,
            status: data.status,
            address: data.address,
            notes: data.notes,
            price: data.price,
            createdAt: data.created_at,
            updatedAt: data.updated_at
        };
    }
    convertSupabasePayment(data) {
        return {
            id: data.id,
            userId: data.user_id,
            appointmentId: data.appointment_id,
            amount: data.amount,
            currency: data.currency,
            status: data.status,
            stripePaymentIntentId: data.stripe_payment_intent_id,
            createdAt: data.created_at,
            updatedAt: data.updated_at
        };
    }
}
exports.DatabaseService = DatabaseService;
// Export singleton instance
exports.databaseService = DatabaseService.getInstance();
//# sourceMappingURL=databaseService.js.map